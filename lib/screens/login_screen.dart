  ...
  Widget build(BuildContext context) {
    ...
    _login({username, password}) async {
      ...
      _services.getAdminCredentials(username).then((value) async {
        print(value.exists);
        // if value.exists is false, it means there is no document with that
        // username. This is a firestore problem. not auth
        print(value['username']);
        // since we fetched a document with id = username, there is no need to
        // check if (value['username'] == username); You can skip this.
        print(value['password']);
        if (value.exists) {
          if (value['username'] == username) { // <- you should skip this.
            if (value['password'] == password) {
              //if both is correct, will login
              try {
                UserCredential userCredential =
                    await FirebaseAuth.instance.signInAnonymously();
                // it is only if the problem occurs here that it is from
                // firebase auth.
